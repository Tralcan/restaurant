...
const RestaurantSchema = z.object({
  ...
  priceLevel: z.enum(['$', '$$', '$$$', '$$$$']).optional().describe("El nivel de precios del restaurante, donde '$' es económico, '$$' es moderado, '$$$' es caro y '$$$$' es muy caro. Omitir si no se puede determinar."),
});
...
const findRestaurantsPrompt = ai.definePrompt({
  ...
  prompt: `Eres una IA buscadora de restaurantes. Encuentra restaurantes basados en la cocina, ciudad y, opcionalmente, la sub-cocina especificada por el usuario.
    ...
    - priceLevel: Una representación textual del nivel de precios (por ejemplo, "$", "$$", "$$$", o "$$$$"). Donde "$" es barato/económico, "$$" es moderado, "$$$" es caro, y "$$$$" es muy caro/lujoso. Si no estás seguro o no puedes determinarlo, puedes omitir este campo.
  `,
});
